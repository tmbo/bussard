...
cat > "$MODEL/groups.toml" <<'EOF'
project = "secure"

groups = [
  { address = "1/2/1", name = "Plain early",     dpt = "5.001" },
  { address = "1/2/3", name = "Secured dimming", dpt = "3.007", secure = true },
  { address = "1/2/4", name = "Plain value",     dpt = "5.001" },
]
EOF
# The product facts and the com-object table live in the lock. Object 1 has R
# and T so the secured group object answers a read and transmits the sim's
# stimulus (issue #172).
cat > "$MODEL/bussard.lock" <<'EOF'
...
secure_capable = true
objects = [
  { number = 1, dpt = "3.007", flags = "CRWT", secure = true },
  { number = 2, dpt = "5.001", flags = "CW" },
]

[[device]]
address = "1.1.20"
objects = [
  { number = 1, dpt = "3.007", flags = "CT" },
]
EOF
...
write_links() {
  local list="" ga
  for ga in "$@"; do list="$list${list:+, }\"$ga\""; done
  {
    printf 'address = "1.1.10"\nname = "Secure dimmer"\n\n'
    printf '[security]\nactivated = true\nsecure_commissioning = false\n\n'
    printf '[links]\n1.listen = ["1/2/3"]\n2.listen = [%s]\n' "$list"
  } > "$MODEL/devices/1.1.10.toml"
  printf 'address = "1.1.20"\nname = "Secured sender (model only)"\n\n[links]\n1.send = "1/2/3"\n' \
    > "$MODEL/devices/1.1.20.toml"
}
write_links 1/2/4
...
printf '[connection]\ntransport = "tunnel"\ngateway = "%s"\n' "$GATEWAY" > "$ASSIGN_MODEL/bussard.toml"
export BUSSARD_KEYRING_PASSWORD="synthetic-keyring-pw"   # SYNTHETIC
mark=$(log_mark)
