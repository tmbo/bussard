...
## 4. Keyring (`.knxkeys`) - byte-exact `[XKNX keyring.py, CONFIRMED]`

There is **no `.knxkeys` file in the repo** `[corpus §4]`, so this is built from
the XKNX reference + synthetic fixtures. The FDSK/seqnum material bussard needs
for v1 already lives in the knxproj (§2.1), so keyring parsing is the second
credential source, not the only one.

### 4.1 Keyring password → key

`PBKDF2-HMAC-SHA256(password_latin1, salt=b"1.keyring.ets.knx.org", iter=65536,
dklen=16)` (§3.4).
...
### 4.3 Password extraction from a decrypted blob

`extract_password(data)`: `length = data[-1]; return data[8:-length]` decoded
UTF-8. Skip the first 8 bytes (salt/prefix), strip PKCS7-style trailing padding
whose count is the last byte. Raw keys (`ToolKey`, `Backbone/@Key`, group
`@Key`) are NOT run through this - they are base64 → raw 16 bytes directly.

### 4.4 Signature verify `[XKNX keyring.py, CONFIRMED - canonicalization fiddly]`

- SAX-walk the XML emitting element markers (byte `1` = start elem, byte `2` =
  end elem), attribute names+values EXCEPT `xmlns` and `Signature`, then append
  the base64 of the hashed keyring password.
- `sha256(serialized)[:16] == base64_decode(Signature)`.
- The signature is a truncated (16-byte) SHA-256 over a canonical serialization.
  The exact byte-serialization order is fiddly - reimplement carefully and diff
  against a synthetic `.knxkeys`. `SEC-CAL: exact .knxkeys signature
  canonicalization byte order (verify against a real/synthetic keyring)`.

### 4.5 Contents (element/attribute names) `[XKNX keyring.py, CONFIRMED]`

- **Backbone**: `Key` (base64→16 raw, multicast key), `MulticastAddress`,
  `Latency` (ms).
- **Interface** (tunnel/USB/backbone): `IndividualAddress`, `Host`, `UserID`,
  `Password` (encrypted → user password string), `Authentication` (encrypted →
  device auth password string), child `GroupAddresses` (`Address`, `Senders`).
- **Device**: `IndividualAddress`, `ToolKey` (base64→16 raw), `ManagementPassword`
  (encrypted), `Authentication` (encrypted), `SequenceNumber` (int, default 0).
- **GroupAddress** (top-level): `Address`, `Key` (base64→16 raw, group key).
...
Keyring {
    ...
    interfaces: Vec<Interface { ia, host, user_id, user_key: Key16, device_auth: Key16, gas: [...] }>,
...
5. `SEC-CAL:` `message_tag` for tunnelling + keepalive/idle timers (§8.3);
   `.knxkeys` signature canonicalization (§4.4).
