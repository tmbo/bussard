...
`groups.yaml` — a map keyed by GA. The header documents the `ranges` key convention
(`"3"` = main group, `"3/2"` = middle group):
...
any number of `listen` GAs, mirroring KNX association semantics. This file is the single
home for the informational com-object `name` (it appears nowhere else):
...
The com-object entries carry no `name` (it lives in `links.yaml`) and no `size` — the
payload size is a pure function of the DPT and is computed on demand; a lowercase
