...
[[product]]
sha256 = "5e0c…"
file = "vendor/Jung_KNX_Aktoren.knxprod"
...
- `[[product]]` (lock v2) records each product-data archive the model
  depends on, identified by `sha256`, its content hash. `file` is the
  archive's path in the model directory (for now under `vendor/`), absent
  when the model does not hold it; `filename` and `size` describe it;
  `applications` lists the programs it carries and `order_numbers` the
  hardware its catalogue maps to one. `origin` says how to get it back:
  `{ kind = "index", order_number, url }` (downloaded through the pointer
  index), `{ kind = "file", path }` (a `.knxprod` given to `import-product`
  or `adopt`), or `{ kind = "knxproj", path, project_hash }` (an ETS project
  export). Until the products of an export are extracted into the model, an
  export entry's `sha256` is the export's own hash, the same as its
  `project_hash`. `import` pins the export it read, `import-product` and the
  product-data download pin the archive, and `adopt` pins the archive it was
  given. A save keeps every entry a device links and every entry that names
  an archive.
...
- `application_id` is the `PID_PROGRAM_VERSION` the device must report, as
  ...
  `flash` refuses an archive it found under `vendor/` whose hash is not the
  one the lock pins; `--product <FILE>` stays the explicit override.
...
bussard writes lock version 2 and reads versions 1 and 2. A v1 lock (no
`[[product]]` entries, no `product_sha256`, `application_id` or
`application_version`) loads unchanged; the next save writes it as v2, and
the next `import` or `import-product` pins the product data.
