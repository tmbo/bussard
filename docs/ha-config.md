...
entities are derived and how to tune the result with `ha.toml`; the command's flags are
...
```yaml
# generated by bussard ha-config — do not hand-edit; use ha.toml overrides
knx:
  switch:
...
each to the GA it sends or listens to (from the device file), groups related objects into a
...
by device channel: it reads GA names from `groups.toml`, strips a known German heating
...
room with an `ha.toml` `merge` on the climate entity's primary GA (the operation-mode
...
Put an optional `ha.toml` next to your model (in the `--dir`) to tune the result. It is
parsed strictly, rejecting unknown fields and duplicate keys, so a typo is an error, not
a silent no-op. The output stays YAML, because that is what Home Assistant reads; only
the overrides file is TOML, like the rest of the model.
...
# Per-entity overrides, one [[entity]] per entry, identified by the entity's
# primary group address (the GA Home Assistant sends to for switch/light/cover,
# or reads for sensor/binary_sensor).
[[entity]]
address = "1/0/1"
platform = "light"          # promote a derived switch to a light (or vice versa)
name = "Kitchen ceiling"    # override the derived name
device_class = "outlet"     # set or override the Home Assistant device_class
...
[[entity]]
address = "0/3/2"           # a climate entity, identified by its operation-mode GA
name = "Büro heating"       # rename it
merge = ["0/3/8"]           # e.g. re-enable a setpoint-shift status GA
```

Two `[[entity]]` entries with the same `address` are an error.
...
`ha.toml`. On the reference model (424 GAs), 393 GAs map (39 switch, 5 light, 26 cover,
