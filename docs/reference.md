...
- `--secure-transport auto|tcp|udp`: the carrier of the KNXnet/IP Secure session (default `auto`: TCP; `udp` is an explicit opt-in, verified against knx-sim only). Needs tunnelling credentials.
...
The model directory is `--dir`, `BUSSARD_DIR` or the current directory; `import` never searches upward. A directory that already holds a model is a re-import and asks nothing. Any other non-empty directory (entries other than `.git`, `.env`, `.gitignore` and `.bussard/`, or a model in `knx/` below it) gets the same question as [`init`](#bussard-init-project): `the current directory /home/me/house is not empty (3 entries); import into the current directory? [y/N]`. `--yes` answers it; without a terminal and without `--yes` the import is refused and nothing is written. To import into the usual `knx/` subdirectory, pass `--dir knx` or `cd knx` first. An import into a directory without a model writes or merges the same `.gitignore` as `init` (`.bussard/`, `*.knxkeys`, `.env`).
...
**A Data Secure-activated device (issue #201).** When the keyring (`--keyring`, else `BUSSARD_KEYRING`, else `connection.keyring` in `bussard.toml`) lists the device in programming mode (password in `BUSSARD_KEYRING_PASSWORD`), adopt treats it as a device ETS has commissioned and only reads it:

- it keeps the device at its address: the keyring looks tool keys up by address, so adopt refuses another target (re-address with `assign --keyring`, then re-export the keyring) and writes no address;
- it verifies the device and reads its link tables, its parameters and its security object over `A_SecureData` with the keyring's tool key, the management path `reconstruct` uses. From the security object it reads the group-object security flags (PID 61) and the security individual address table (PID 54);
- the device file gets the links (an object with the `T` flag sends on its first associated address and listens on the rest; others listen), the parameters that differ from the vendor defaults, and `[security] activated = true, secure_commissioning = true`. The lock gets `secure_capable = true`, the keyring's sequence number and the PID 54 table as `secure_senders`, which a later secured `flash` or `apply` writes back next to the senders the links imply. The objects PID 61 flags are marked `secure` (the keyring's group keys decide when PID 61 is not readable), and so are the linked group addresses the keyring holds a key for; a group address the model lacks is created as `GA <address> (adopted from <ia>)`. Where PID 61 and the keyring disagree, the summary says which way;
- nothing is written to the device. It stays in programming mode until its button is pressed. `bussard plan <ia>` right after reports `matches the model`.

A device that hides its mask from the unsecured read (mask `FFFF`) but is not in the keyring fails with "the keyring … has no tool key for <ia>" and a hint to re-export the keyring from ETS; no device file is written. Activating a factory-fresh Secure device (the FDSK certificate) is ETS's job (#201 tier 2).
...
The pre-flight and the write phase share one tunnel. With `--yes` they also share the device connection: the write phase takes over the pre-flight's connection instead of disconnecting and connecting again (issue #213), which saves a `T_Connect`, the Data Secure `S-A_Sync` and the `A_Authorize_Request`. The frames written are the same. Without `--yes` the confirmation prompt sits between the phases, and a device closes a connection that is silent for 6 s, so the write phase reconnects and reuses what the pre-flight learned (mask, max APDU, object table, authorize verdict) instead of reading it again. A connection that was idle for more than 3 s or that the device closed is replaced by a fresh one; `BUSSARD_FLASH_NO_HANDOVER=1` always reconnects.
...
| `BUSSARD_SECURE_KEEPALIVE_SECS` | The KNXnet/IP Secure session keepalive interval in seconds (default `30`; `0` sends none). A wrapped SESSION_STATUS `STATUS_KEEPALIVE` goes out this often. The default is half the 60 s session timeout measured on the Jung interface (the KNX specification's value); `bussard test --secure-idle` measures another interface's. A value of 60 or more prints a warning at connect. |
...
`authorize = "unsupported"` lets `describe`, `plan`, `reconstruct`, the read phase of `apply` and the `flash` pre-flight skip `A_Authorize_Request`, which such a device leaves unanswered for the full response timeout (3 s per session). When the facts turn out stale on that connection the key is presented after all. The write phase of `apply` and `flash` then relies on the verdict its own read phase established. A device recorded as `granted` or `denied` (it answers, possibly asking for a key) is always asked, and `flash --bcu-key` always presents the key. The written frames of `apply` and `flash` do not change.
...
| `knx_describe_device` | `address` | Introspects a device: enumerates its interface objects and each property's description (PID, type, element count, access levels). Read-only on the bus. With `--keyring`, a KNX Data Secure device is addressed with its tool key. The server keeps the connection for 3 s after a call, so a following call to the same device reuses it: no second `T_Connect`, `S-A_Sync`, authorize or max-APDU read. A call to another device, a programming-tier call, a lost gateway link or the idle window closes it first; a call that fails on the kept connection is repeated on a fresh one. Omitted in `--passive` mode. |
| `knx_infer_group` | `ga`, `payload_hex` (optional) | What the traffic on a GA says it is: ranked DPT candidates (`dpt`, `confidence` of `low`/`medium`/`high`, `reason`), the sender (address, name, location), its channel and com object (index, name, declared DPT, flags), the GA's current model entry, a proposed name, and a `next_step` telling the assistant to confirm with the human before calling `knx_set_group` and `knx_add_link`. Uses every telegram seen on the GA, or `payload_hex` when given. A secured telegram counts only when the server's keyring decrypted it (`secured: true`); `undecrypted_secured` counts the ones left out and `secure_note` says why. Reads only; also registered in `--passive`. |
...
| `knx_plan_device` | `address` | Reads the device's live tables (read-only on the bus) and returns the plan `bussard plan --json` has for the tables: `sentences` (the text rendering in the device file's words), `changes`, `unchanged_objects`, `writes`, `question`, `state_hash`, plus `plan` (the table-level detail: additions, removals, unchanged count, table sizes, load operations), `pending_model_changes` (sentences), the same lists as JSON, `backup_dir`, `plan_digest`, `planned_at` and `expires_at`. Parameters are not compared over MCP. A plan with nothing to do has no digest. Refuses a change that touches a protected GA. With the server's keyring, a Data Secure device the keyring lists is read over KNX Data Secure (`secured: true`), and `security_object` says what its security object receives with the write: `summary` (the line `bussard apply --keyring` prints), `secure_senders`, `keyed_groups` and `secured_objects`, never a key. A group the model marks `secure` without a key in the keyring refuses the plan. Registered only with `--allow-programming`. |
| `knx_apply_device` | `address`, `plan_digest`, `plan_hash` (optional) | Writes the planned tables: backup first, then the `bussard apply` write, then a read-back verify. Returns `verified`, the final load states, `backup`, `gateway` and the history `snapshot`. Refused unless the digest is fresh and a new read still reproduces it (see below), and when `plan_hash` is given and a fresh read no longer hashes to it. A device the server's keyring lists is written over KNX Data Secure, and on System B its security object is reprogrammed with the tables (security individual address table, group key table, group-object flags), exactly as `bussard apply --keyring` does; the result carries `secured` and `security_object`. Registered only with `--allow-programming`. |
...
- **Plan digest.** `knx_plan_device` returns `plan_digest`, a SHA-256 over the device address, the model's links for it, the desired tables, what a secured write gives the security object (addresses, object numbers and sequence numbers, no key) and the live tables it read. `knx_apply_device` refuses unless the digest came from this server session within `--plan-ttl-minutes` (default 10) and a fresh read of the device, with the current model, reproduces it. A digest is single use: a write, or a refusal because the device or the model moved, retires it.
...
- **Inspector.** The right-hand panel shows the selected device's com objects grouped by channel (object name, DPT, flags, the send and listen GAs and the devices on the other end), or the selected GA's senders and listeners. Under each channel, after its com objects, come the parameter values the device file sets there (device-level ones under "General"), read-only: the vendor's label, the value (the choice label for an enumeration) and a "changed" marker when it differs from the vendor default. Labels and defaults come from the product model in `.bussard/models/`; without it the page shows the file key and the value as written. It opens 780 px wide, enough for the widest device in the synthetic fixture with room for longer names; longer labels wrap inside the table rather than scroll sideways. Drag the bar between the diagram and the panel to resize it (or focus it and use the arrow keys, double-click for the default). It stays between 360 px and the window width minus 480 px for the diagram, and the browser remembers the width.
...
| `GET /api/model` | The precomputed model projection (devices, groups, ranges, links, senders/listeners, and per device and channel the `parameters` rows `{key, text, value, default, non_default}`; `text`, `default` and `non_default` are `null` without the product model). Rebuilt in place by `POST /api/reload`. |
