...
///
/// ```text
/// [wire] 2026-09-24T10:15:00.123Z TX >> 1.0.255 -> 1.0.30  A_PropertyValue_Read  APCI=0x3D5  apdu=[4b d5 00 0c 10 01]  cemi=11 00 …
/// ```
///
/// where the timestamp is the wall-clock time the frame was traced (ISO-8601
/// UTC, milliseconds; issue #194), so per-frame timing can be measured from a
...
///
/// Hand-rolled (Howard Hinnant's civil-from-days, as `bussard-monitor` and
/// `bussard-download` use) so the transport crate needs no date-time crate. A
/// clock before the Unix epoch prints the epoch.
fn utc_millis(ts: std::time::SystemTime) -> String {
    let ms = ts
...
mod tests {
    ...
    #[test]
    fn test_utc_millis_formats_iso8601_with_milliseconds() {
        let at = |ms: u64| std::time::UNIX_EPOCH + std::time::Duration::from_millis(ms);
        assert_eq!(utc_millis(at(0)), "1970-01-01T00:00:00.000Z");
        // 2026-09-24T10:15:00.123Z
        assert_eq!(
            utc_millis(at(1_790_244_900_123)),
            "2026-09-24T10:15:00.123Z"
        );
        // A leap day.
        assert_eq!(
