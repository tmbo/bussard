...
fn write_wrapper(path: &Path, entries: &[(&str, &[u8])]) {
...
#[test]
fn test_read_knxprod_transparent_wrapper_equals_direct() {
    let tmp = tempfile::tempdir().unwrap();
    let inner = knxprod_bytes();

    // Direct read of the plain `.knxprod`.
    let direct_path = tmp.path().join("direct.knxprod");
    std::fs::write(&direct_path, &inner).unwrap();
    let direct = bussard_prod::read_knxprod(&direct_path).unwrap();
    ...
    let wrapped = bussard_prod::read_knxprod(&wrapped_path).unwrap();
...
#[test]
fn test_read_knxprod_wrapper_at_root() {
    let tmp = tempfile::tempdir().unwrap();
    let inner = knxprod_bytes();
...
#[test]
fn test_read_knxprod_multi_inner_lists_entries() {
    let tmp = tempfile::tempdir().unwrap();
    let inner = knxprod_bytes();
    ...
    let err = bussard_prod::read_knxprod(&path).unwrap_err().to_string();
...
#[test]
fn test_read_knxprod_inner_selects_by_name() {
    let tmp = tempfile::tempdir().unwrap();
    let inner = knxprod_bytes();
    let path = tmp.path().join("multi.zip");
    write_wrapper(
    ...
    let product = bussard_prod::read_knxprod_inner(&path, Some("second.knxprod")).unwrap();
    assert_eq!(product.applications.len(), 1);

    // Full entry path also matches.
    let product = bussard_prod::read_knxprod_inner(&path, Some("pkg/first.knxprod")).unwrap();
    assert_eq!(product.applications.len(), 1);
...
#[test]
fn test_read_knxprod_no_inner_knxprod() {
    let tmp = tempfile::tempdir().unwrap();
...
#[test]
fn test_read_knxprod_nested_wrapper_rejected() {
    let tmp = tempfile::tempdir().unwrap();
    let inner = knxprod_bytes();
    ...
    let mut middle = Vec::new();
    {
        ...
        let opts = SimpleFileOptions::default();
        zip.start_file("inner.knxprod", opts).unwrap();
...
#[test]
fn test_read_knxprod_oversized_inner_rejected() {
    let tmp = tempfile::tempdir().unwrap();
    ...
    zip.start_file("huge.knxprod", opts).unwrap();
