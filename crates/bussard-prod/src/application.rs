//! Streaming parser for one manufacturer ApplicationProgram XML file.
//!
//! These files reach 28 MB, so we never build a DOM: `quick-xml` pulls events
//! and we accumulate typed records. The full set phase-2/3 needs is extracted —
//! identity, the com-object table, parameter types, parameters and their refs,
//! code segments (metadata only), and the load procedure — while binary blobs
//! (`<Baggages>`, absolute-segment payload data) are ignored.
//!
...
pub fn parse_application_program(id: &str, xml: &str) -> Result<ApplicationProgram> {
    ...
    // Translation state: (language, refid, attribute) -> text; we keep only
    // en-US and apply after parsing.
    let mut translations: HashMap<(String, String), String> = HashMap::new();
    let mut cur_lang: Option<String> = None;
    let mut cur_tu: Option<String> = None;
    ...
    loop {
        let ev = reader.read_event().map_err(|source| ProdError::Xml {
        ...
        match ev {
            ...
            Event::End(e) => match e.local_name().as_ref() {
                b"Language" => cur_lang = None,
                b"TranslationElement" => cur_tu = None,
...
fn handle_start(
    ...
    cur_lang: &mut Option<String>,
    cur_tu: &mut Option<String>,
    translations: &mut HashMap<(String, String), String>,
    ...
    match e.local_name().as_ref() {
        ...
        b"Language" => {
            *cur_lang = get(&m, b"Identifier").map(str::to_string);
        }
        b"TranslationElement" => {
            *cur_tu = get(&m, b"RefId").map(str::to_string);
        }
        ...
        name if name.starts_with(b"LdCtrl") => {
            push_load_op(cur_lp, e, &m, context)?;
        }
        _ => {}
    }
    ...
    if e.local_name().as_ref() == b"Translation" {
        capture_translation(&m, cur_lang, cur_tu, translations);
    }
    Ok(())
}
...
fn handle_empty(
    ...
    match e.local_name().as_ref() {
        ...
        b"Translation" => capture_translation(&m, cur_lang, cur_tu, translations),
...
/// Captures an en-US `<Translation AttributeName="Name"/"Text">` value.
fn capture_translation(
    m: &HashMap<Vec<u8>, String>,
    cur_lang: &Option<String>,
    cur_tu: &Option<String>,
    translations: &mut HashMap<(String, String), String>,
) {
    if cur_lang.as_deref() != Some("en-US") {
        return;
    }
    let (Some(tu), Some(attr_name), Some(text)) =
        (cur_tu, get(m, b"AttributeName"), get(m, b"Text"))
    else {
        return;
    };
    if attr_name == "Name" || attr_name == "Text" {
        translations.insert((tu.clone(), attr_name.to_string()), text.to_string());
    }
}
...
fn apply_translations(
    ...
    // Application name.
    if let Some(t) = translations.get(&(app.id.clone(), "Name".to_string())) {
        app.name = Some(t.clone());
    }
    for (id, obj) in app.com_objects.iter_mut() {
        if let Some(t) = translations.get(&(id.clone(), "Text".to_string())) {
            obj.text = Some(t.clone());
        }
    }
    for (id, cref) in app.com_object_refs.iter_mut() {
        if let Some(t) = translations.get(&(id.clone(), "Text".to_string())) {
            cref.text = Some(t.clone());
        }
    }
    for (id, param) in app.parameters.iter_mut() {
        if let Some(t) = translations.get(&(id.clone(), "Text".to_string())) {
            param.text = Some(t.clone());
        }
    }
}
...
/// Parses all attributes of a tag into a map keyed by attribute name.
fn attrs_map(e: &BytesStart, context: &str) -> Result<HashMap<Vec<u8>, String>> {
    let mut map = HashMap::new();
    for attr in e.attributes() {
        let attr = attr.map_err(|source| ProdError::XmlAttr {
            context: context.to_string(),
            source,
        })?;
        let value = attr
            .unescape_value()
            .map_err(|source| ProdError::Xml {
                context: context.to_string(),
                source,
            })?
            .into_owned();
