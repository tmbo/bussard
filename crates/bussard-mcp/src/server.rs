...
//! `knx_write_group` and `knx_run_tests` write tools (twelve total) over the
//! Model Context Protocol. The learn and acceptance tools live in their own
//! router in [`crate::tools_learn`], combined in [`BussardMcp::new`].
//! Each `#[tool]`
...
impl BussardMcp {
    ...
    pub fn new(state: Arc<SharedState>) -> Self {
        // The learn/acceptance tools live in their own router module
        // (`tools_learn`); rmcp's `ToolRouter` adds, so the two are combined
        // here and then trimmed for this server's tier.
        let mut tool_router = Self::tool_router() + Self::learn_router();
        if state.passive {
