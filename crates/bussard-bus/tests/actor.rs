...
/// A mock that stalls the ACK for the client's send while pushing an inbound
/// indication meanwhile. It proves the actor keeps delivering inbound frames
/// while a send's (slow) ACK is outstanding (issue #57): the old inline
/// `conn.send().await` in the select held the recv arm for the whole ACK window.
///
/// Sequence: handshake, then on the first TUNNELING_REQUEST push an indication
/// to the client immediately, wait `ack_delay`, and only then ACK the request.
async fn run_mock_delay_ack_with_push(
    gw: UdpSocket,
    ack_delay: Duration,
    g: GroupAddress,
    ...
    loop {
        ...
        match parsed.service {
            ...
            ServiceType::TunnelingRequest => {
                ...
                if !handled_first {
                    handled_first = true;
                    // Push an inbound indication first, then stall before ACKing
                    // the client's send. A subscriber must see this indication
                    // long before the send receipt resolves.
                    let hdr = ConnectionHeader {
                        channel_id: CHANNEL,
                        seq: 0,
                    };
...
#[tokio::test]
async fn inbound_delivered_while_send_ack_is_blocked() {
    // The gateway stalls the ACK for the client's send by ~800ms while pushing an
    // indication immediately. The indication must fan out to the subscriber well
    // before the send receipt resolves — proving `conn.recv()` keeps being polled
    // during a slow send (issue #57).
    let (addr, gw) = bind_mock().await;
    let ack_delay = Duration::from_millis(800);
    let gw_task = tokio::spawn(run_mock_delay_ack_with_push(
        gw,
        ack_delay,
        ga("7/1/1"),
        ia("1.1.20"),
    ...
    // Kick off a send; its ACK is stalled ~800ms by the mock.
    ...
    // The pushed indication must arrive quickly (well under the ACK delay). If
    // the recv arm were starved by the inline send, this would block ~800ms.
    let started = std::time::Instant::now();
    let frame = tokio::time::timeout(Duration::from_millis(400), sub.recv())
        .await
        .expect("inbound frame delivered while the send ACK is still blocked")
        .expect("subscriber channel open");
    let elapsed = started.elapsed();
    assert_eq!(
        frame.frame.frame.group_destination().unwrap().to_string(),
        "7/1/1"
    );
    assert!(
        elapsed < ack_delay,
        "inbound must arrive before the send ACK ({elapsed:?} vs {ack_delay:?})"
    );

    // The send still completes (its ACK eventually arrives).
    let receipt = tokio::time::timeout(Duration::from_secs(3), send_task)
        .await
