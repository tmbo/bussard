...
/// The only `bussard.lock` format version this build reads and writes (issue
/// #228: v2 adds the `[[product]]` entries and the per-device product hash and
/// application id). bussard is pre-release: another version is refused, and
/// `bussard import` regenerates the lock.
