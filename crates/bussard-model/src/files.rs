...
use crate::schema::{
    ...
    Product, ProductEntry, SecureSender, Spelling,
};
use crate::toml_io::{self, ParseError};

/// The `bussard.lock` format version this build writes (issue #228: v2 adds
/// the `[[product]]` entries and the per-device product hash and application
/// id).
pub const LOCK_VERSION: u32 = 2;

/// Every `bussard.lock` format version this build reads. A v1 lock loads
/// unchanged and is written as v2 on the next save.
pub const LOCK_VERSIONS_READ: [u32; 2] = [1, 2];
...
pub(crate) fn join_device(
    ...
    if let Some(lock) = lock {
        ...
        extras.product_sha256 = lock.product_sha256.clone();
        // The id and version the application ref encodes are rederived on
        // save; only a value that differs from them is kept.
        let derived = lock
            .application
            .as_deref()
            .and_then(crate::identity::application_id_from_ref);
        if lock.application_id.as_deref() != derived.as_ref().map(|(id, _)| id.as_str()) {
            extras.application_id = lock.application_id.clone();
        }
        if lock.application_version != derived.as_ref().map(|(_, v)| *v) {
            extras.application_version = lock.application_version;
        }
...
/// The `[[product]]` entries a save writes (lock v2, issue #228): every entry
/// a device links, plus the entries of the lock being replaced that name an
/// archive the model holds (`file`) or that a device still links, so product
/// identity is never lost by a save. A link whose entry no longer lists the
/// device's application is dropped (the application changed).
pub(crate) fn lock_products<'a>(
    locks: &mut [LockDevice],
    devices: impl Iterator<Item = &'a Device>,
    existing: Option<&str>,
) -> Vec<ProductEntry> {
    let mut products: BTreeMap<String, ProductEntry> = BTreeMap::new();
    for device in devices {
        if let Some(entry) = &device.lock.product_entry {
            products
                .entry(entry.sha256.to_ascii_lowercase())
                .or_insert_with(|| entry.clone());
        }
    }
    let previous: Vec<ProductEntry> = existing
        .and_then(|text| toml::from_str::<LockFile>(text).ok())
        .map(|lock| lock.products)
        .unwrap_or_default();
    let linked: BTreeSet<String> = locks
        .iter()
        .filter_map(|l| l.product_sha256.as_deref().map(str::to_ascii_lowercase))
        .collect();
    for entry in previous {
        let sha = entry.sha256.to_ascii_lowercase();
        if entry.file.is_some() || linked.contains(&sha) {
            products.entry(sha).or_insert(entry);
        }
    }
    // A device without a link takes the archive the lock pins for its order
    // number and application, so `adopt` (and any save after an
    // `import-product`) records where its product data is.
    for lock in locks.iter_mut() {
        if lock.product_sha256.is_some() {
            continue;
        }
        let Some(order) = lock.product.as_deref() else {
            continue;
        };
        let serves = |entry: &&ProductEntry| {
            entry.file.is_some()
                && entry
                    .order_numbers
                    .iter()
                    .any(|o| o.trim().eq_ignore_ascii_case(order.trim()))
                && lock.application.as_deref().is_none_or(|app| {
                    entry.applications.is_empty() || entry.applications.iter().any(|a| a == app)
                })
        };
        if let Some(entry) = products.values().find(serves) {
            lock.product_sha256 = Some(entry.sha256.to_ascii_lowercase());
        }
    }
    for lock in locks.iter_mut() {
        let Some(sha) = lock.product_sha256.as_deref() else {
            continue;
        };
        let stale = products
            .get(&sha.to_ascii_lowercase())
            .zip(lock.application.as_deref())
            .is_some_and(|(entry, app)| {
                !entry.applications.is_empty() && !entry.applications.iter().any(|a| a == app)
            });
        if stale {
            lock.product_sha256 = None;
        }
    }
    let mut out: Vec<ProductEntry> = products.into_values().collect();
    out.sort_by(|a, b| {
...
pub(crate) fn lock_entry(device: &Device) -> Option<LockDevice> {
    ...
    let entry = LockDevice {
        ...
        application_version: device
            .lock
            .application_version
            .or_else(|| derived.as_ref().map(|(_, v)| *v)),
        application_id: device
            .lock
            .application_id
            .clone()
            .or_else(|| derived.map(|(id, _)| id)),
        application,
